...
class User(db.Model):
    ...
    @property
    def password(self):
        raise AttributeError('пароль - це нечитливий атрибут')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            ...
            if existing_email:
                flash('Користувач з цією електронною адресою вже існує. Будь ласка, використовуйте іншу електронну адресу.', 'danger')
            else:
                if not form.validate_username(form.username):
                    ...
                    if form.password.data != form.confirm_password.data:
                        flash('Пароль та підтвердження не збігаються.', 'danger')
                    else:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.verify_password(form.password.data):
            ...
            flash('Неправильна електронна адреса або пароль. Будь ласка, спробуйте ще раз.', 'danger')
